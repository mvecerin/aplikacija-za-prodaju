...
public class UnosPrivatnogKorisnikaController {
	...
	public void unesi() {
		...
		if (imeText.getText().contentEquals("")) {
			errorString += "Ime je obavezan podatak!\n";
		}
		if (prezimeText.getText().contentEquals("")) {
			errorString += "Prezime je obavezan podatak!\n";
		}
		...
		if(errorString.contentEquals("")) {
			...
			OptionalLong maxId = PrivatniKorisniciController.korisnici.stream().mapToLong(Entitet::getId).max();
			Long idKorisnik = maxId.getAsLong()+1;
			try {
				BazaPodataka.pohraniNovogPrivatnogKorisnika(new PrivatniKorisnik(imeText.getText(), prezimeText.getText(), emailText.getText(), telefonText.getText(), idKorisnik));
